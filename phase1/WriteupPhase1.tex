...
Therefore we know already that in the category \textbf{Set} isomorphisms are exactly those maps that are both monic and epic.
We now show for a general category $\mathscr{C}$ that every isomorphism is monic and epic.
\begin{proof}
...
However monic and epic maps do not necessarily have inverses.\\
If a map $f$ is an isomorphism, then it has a unique inverse. Thus we speak of
\emph{the} inverse of $f$.

\begin{proof}
  Let $f \from A \to B$ be a map.
  Assume there exist two inverses $g \from B \to A$: $gf = 1_A$, $fg = 1_B$ and $h \from B \to A$: $hf = 1_A$, $fh = 1_B$.
